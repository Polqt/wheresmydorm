...
import { listingPhotos, listings } from "./listings";
import {
  conversationReports,
  inquiryStatuses,
  messages,
...
import { reviewHelpfulVotes, reviewReports, reviews } from "./reviews";
import {
  follows,
  postComments,
  postReactions,
...
export const inquiryStatusesRelations = relations(
  inquiryStatuses,
  ({ one }) => ({
    finder: one(profiles, {
      fields: [inquiryStatuses.finderId],
      references: [profiles.id],
    }),
    lister: one(profiles, {
      fields: [inquiryStatuses.listerId],
      references: [profiles.id],
    }),
    listing: one(listings, {
      fields: [inquiryStatuses.listingId],
      references: [listings.id],
    }),
  }),
);
...
export const conversationReportsRelations = relations(
  ...
  ({ one }) => ({
    listing: one(listings, {
      fields: [conversationReports.listingId],
      references: [listings.id],
    }),
    reporter: one(profiles, {
    ...
    reportedUser: one(profiles, {
      fields: [conversationReports.reportedUserId],
      references: [profiles.id],
    }),
  }),
);
...
export const searchEventsRelations = relations(searchEvents, ({ one }) => ({
  user: one(profiles, {
    fields: [searchEvents.userId],
    references: [profiles.id],
  }),
  listing: one(listings, {
    fields: [searchEvents.listingId],
    references: [listings.id],
  }),
}));
