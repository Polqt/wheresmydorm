...
function getStatusTone(status: ModerationStatus) {
  switch (status) {
    case "actioned":
      return { bg: "#FEF2F2", label: "Actioned", text: "#B91C1C" };
    case "dismissed":
      return { bg: "#F5F0EA", label: "Dismissed", text: "#78716C" };
    case "reviewed":
      return { bg: "#EFF6FF", label: "Reviewed", text: "#1D4ED8" };
    default:
      return { bg: "#FFF7ED", label: "Pending", text: "#C2410C" };
  }
}
  ...
  return (
    <View style={styles.card}>
      ...
      <View style={styles.actionsRow}>
        <Pressable
          onPress={() => router.push(postDetailRoute(item.postId))}
          style={styles.secondaryAction}
        >
          <Text style={styles.secondaryActionText}>Open post</Text>
        </Pressable>
        <Pressable
      ...
      <View style={styles.actionsRow}>
        ...
        >
          <Text style={styles.primaryActionText}>
            {isModerating ? "Updating..." : "Remove post"}
          </Text>
        </Pressable>
      </View>
    </View>
...
export default function PostReportsScreen() {
  const { role } = useAuth();
  ...
  const reportsQuery = useQuery({
    ...trpc.admin.listPostReports.queryOptions(),
    enabled: role === "admin",
  });
      ...
      onSuccess: async (_, variables) => {
        await Promise.all([
          queryClient.invalidateQueries({
            queryKey: ["trpc", "admin", "listPostReports"],
          }),
          queryClient.invalidateQueries({
        ...
        setFeedback({
          tone: "success",
          message: variables.removePost
            ? "Post removed and report actioned."
            : "Post report updated.",
        });
        setActiveReportId(null);
      },
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Post reports</Text>
      ...
      <View style={styles.summary}>
        ...
        <Text style={styles.summaryBody}>
          Review reported posts and remove content when moderation requires it.
        </Text>
      </View>
      {feedback ? (
        <View
      ...
      <FlashList
        ...
        renderItem={({ item }) => (
          <ReportCard
            ...
            onModerate={(input) => {
              setFeedback(null);
              setActiveReportId(input.reportId);
              moderateReport.mutate(input);
            }}
          />
        )}
      />
